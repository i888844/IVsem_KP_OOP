...
string parse_period(const string& _period)
{
    const string month = _period.substr(0, _period.find(" "));
    const string year = _period.substr(_period.find(" ") + 1, 4);

    const string month_number = parse_month_to_number(month);
...
public:
    ...
    void output() const
    {
        cout << parse_period(period) << "  " << type << "  " << sum << endl;
    }
};
...
public:
    ...
    void output() const
    {
        cout << parse_period(get_period()) << "  " << get_type() << "  " << get_sum() << "  " << attendance << "  " << hours << endl;
    }
};
